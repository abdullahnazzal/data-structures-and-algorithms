...
const extractAbilities = (arr) => {
  ...
  arr.map((item)=>{
    // console.log(item);
    newArray.push(item.ability.name);
  })
  return newArray;
};
...
const extractStats = (arr) => {
  let newArray=[];
  arr.map((item)=>{
    // console.log("saadsadsa============="+{"name":item.stat.name,"total":item.effort+item.baseStat});
    newArray.push({"name":item.stat.name,"total":item.effort+item.baseStat});
  })
